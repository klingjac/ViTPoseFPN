load_from = '/scratch/rob599w25_class_root/rob599w25_class/klingjac/ViTPose/base_weights/backbone_only_with_prefix.pth'
_base_ = [
...
model = dict(
    ...
    backbone=dict(
        ...
        out_indices=[9, 10, 11],  # Keep output at 3, 7, 11
        frozen_stages=11,
        freeze_attn=True,
        freeze_ffn=True
    ),
    keypoint_head=dict(
        type='TopdownHeatmapSimpleHeadFPN',
        in_channels=[768, 768, 768],
        in_index=[0, 1, 2],  # use feature at 3 (lowest-res), 7, and 11 (highest-res)
        input_transform='multiple_select',
        # fpn_strides=[4, 2, 1],  # <-- last feature (index 2) has stride 1 (highest resolution)
        num_deconv_layers=2,
...
data = dict(
    ...
    val=dict(
        type='TopDownCocoDataset',
        ann_file=f'data/coco/annotations/person_keypoints_val2017.json',
